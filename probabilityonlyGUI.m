function [RowArray,ColumnArray,HitArray] = probabilityonlyGUI(handles)
board=CreateChoiceArray(handles);
...
RowArray = zeros(1,100);
ColumnArray = zeros(1,100);
HitArray = zeros(1,100);
...
while nOccupied >0
    ...
    if board (rowMax, columnMax) == 0
        ...
        nTurns = nTurns + 1;
        RowArray(nTurns) = rowMax;
        ColumnArray(nTurns) = columnMax;
        HitArray(nTurns) = 0;
